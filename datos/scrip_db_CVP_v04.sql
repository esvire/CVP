...
CREATE TABLE IF NOT EXISTS `db_CVP`.`tbl_rol` (
  `id_rol` INT NOT NULL AUTO_INCREMENT COMMENT 'este campo contiene la clave primaria de la tabla rol',
...
CREATE TABLE IF NOT EXISTS `db_CVP`.`tbl_usuario` (
  `id_usuario` INT NOT NULL AUTO_INCREMENT COMMENT 'este campo contiene la clave primaria de la tabla usuario.',
  ...
  `pregunta` VARCHAR(100) NULL COMMENT 'Pregunta para proseso de recuperacion de contraseña',
  `respuesta` VARCHAR(100) NULL COMMENT 'Respueta para proceso ',
...
-- -----------------------------------------------------
-- Table `db_CVP`.`tbl_parqueadero`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_CVP`.`tbl_parqueadero` (
  `id_Parqueadero` INT NOT NULL AUTO_INCREMENT,
  `nombre` VARCHAR(45) NULL COMMENT 'Nombre de el parqueadero ',
  `nit` VARCHAR(45) NULL COMMENT 'NIT del parqueadero',
  `Direccion` VARCHAR(45) NULL COMMENT 'Dirección del parqueadero',
  `telefono` VARCHAR(45) NULL COMMENT 'teléfono del parquadero',
...
CREATE TABLE IF NOT EXISTS `db_CVP`.`tbl_tipo_vehiculo` (
  `id_tipo_vehiculo` INT NOT NULL AUTO_INCREMENT COMMENT 'este campo contiene la clave primaria de la tabla tarifa',
  ...
  CONSTRAINT `fk_tbl_tipo_vehiculo_tblL_parqueadero1`
    ...
    REFERENCES `db_CVP`.`tbl_parqueadero` (`id_Parqueadero`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `db_CVP`.`tbl_vehiculo` (
  `id_vehiculo` INT NOT NULL AUTO_INCREMENT COMMENT 'Este campo contiene la clave foranea de la tabla tipo de vehiculo.',
  `placa` VARCHAR(45) NOT NULL COMMENT 'En este campo se guarda l amatricula delccarro.',
  `estado` INT NOT NULL COMMENT 'este campo es el estado del vehiculo.',
...
CREATE TABLE IF NOT EXISTS `db_CVP`.`tbl_cliente` (
  `id_cliente` INT NOT NULL AUTO_INCREMENT COMMENT 'este sera un campo ocasional en el que depende el tipo de servicio se registrara el campo del cliente.',
...
CREATE TABLE IF NOT EXISTS `db_CVP`.`tbl_servicio` (
  `id_servicio` INT NOT NULL AUTO_INCREMENT COMMENT 'este campo contiene la calve primaria de la tabla servicio.',
...
CREATE TABLE IF NOT EXISTS `db_CVP`.`tbl_celda` (
  `id_celda` INT NOT NULL AUTO_INCREMENT COMMENT 'este campo contiene la clave primaria de la tabla (celda)	',
  `numero` INT NOT NULL DEFAULT 0 COMMENT 'este campo indica la camidad de celdas disponibles',
  `estado` INT NOT NULL DEFAULT 1 COMMENT 'en este campo se indica el estado de cada celda (ocupada o llibre)',
...
CREATE TABLE IF NOT EXISTS `db_CVP`.`tbl_regimen` (
  `id_regimen` INT NOT NULL AUTO_INCREMENT COMMENT 'este campo contiene la clave primaria de la tabla tegimen.',
  `fechaInicio` DATE NOT NULL COMMENT 'Este campo contiene la fecha de inicio de dicho regimen.			',
  `fechaFinal` DATE NOT NULL COMMENT 'este campo contiene  la fecha final del regimen.',
...
CREATE TABLE IF NOT EXISTS `db_CVP`.`tbl_factura` (
  `id_factura` INT NOT NULL AUTO_INCREMENT COMMENT 'Este campo contiene la clave primaria de la tabla factura.',
  `subtotal` DOUBLE NOT NULL COMMENT 'Subtotal Valor del servicio',
  `iva` DOUBLE NULL COMMENT 'total cobrado por IVA',
  `descuento` DOUBLE NULL,
  `total` DOUBLE NOT NULL COMMENT 'en este campo se calculara el total de los servicios.',
  ...
  `tbl_servicio_id_servicio` INT NOT NULL,
...
--           insertar Roles            --
-- ----------------------------------- --
insert into tbl_rol(tipoRol, estado) values('administrador', 1);
insert into tbl_rol(tipoRol, estado) values('operario', 1);

-- ----------------------------------- --
--   insertar Administrador defoult    --
-- ----------------------------------- --
INSERT INTO tbl_usuario(nombre, apellido, documento, direccion, telefono, clave, pregunta, respuesta, estado_usuario,tbl_rol_id_rol) 
	VALUES('administrador', 'gerencia', '888', '888', '123cr123', '312123321', 'nombre de equipo?', 'millonarios',1,1); 

-- ----------------------------------- --
--    insertar Parqueadero defoult     --
-- ----------------------------------- --
INSERT INTO tbl_parqueadero(nombre, nit, direccion, telefono,estado) 
	VALUES('Parqueadero', '#11111111', 'Car00 # 00-00', '000 000 000', 1); 


-- ----------------------------------- --
--      insertar Cliente defoult       --
-- ----------------------------------- --
INSERT INTO tbl_cliente(nombre, apellido, documento, telefono, estado_cliente) 
	VALUES('Ocacional', 1,'#11111111', '000 000 000', 1); 
